...
class DrawRectangle(AbstractShape):
    ...
    def draw(self, **kwargs):
        ...
        if self.cRectangle == 0:
            k1x, k2x = np.max(self.keyPoint1, axis=0), np.max(self.keyPoint2, axis=0)
            k1n, k2n = np.min(self.keyPoint1, axis=0), np.min(self.keyPoint2, axis=0)
        ...
        # self.image = new_image
            ...
            ImageList, x = np.empty(0), 0
            reclist1, reclist2, reclist3 = np.empty(shape=[0, 2]), np.empty(shape=[0, 2]), np.empty(shape=[0, 2])
            for k1, k2 in zip(self.keyPoint1, self.keyPoint2):
                if len(self.keyPoint1) > 1:
                    sta = (k1[0] - k2[0]) / (k1[1] - k2[1])
                    sta = int(sta)
                    ImageList = np.append(ImageList, [sta])
            mode = stats.mode(ImageList)

            while x != len(ImageList):
                if ImageList[x] == mode[0]:
                    ImageList = np.delete(ImageList, x)
                else:
                    x = x + 1
            mode2 = stats.mode(ImageList)
            for k1, k2 in zip(self.keyPoint1, self.keyPoint2):
                if len(self.keyPoint1) > 1:
                    sta = (k1[0] - k2[0]) / (k1[1] - k2[1])
                    sta = int(sta)
                    ...
                    elif sta == mode2[0] or mode2[1] * 2 <= mode[1]:
